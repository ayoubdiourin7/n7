...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (match (accept EqualToken stream) with
       ...
       | _ -> Failure)
    (* regle #7  - EX ->          | { $, ) } *)
    | EOSToken | RightParenthesisToken -> (print_endline "");(Success stream)
    | _ -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    (* regle #13 - FX -> * F FX   | { * } *)
    | StarToken -> (print_endline "* F FX");(match (accept StarToken stream) with
       | Success next -> (match (parseF next) with
          | Success next2 -> (parseFX next2)
          | _ -> Failure)
       | _ -> Failure)
    (* regle #14 - FX -> / F FX   | { / } *)
    | SlashToken -> (print_endline "/ F FX");(match (accept SlashToken stream) with
       | Success next -> (match (parseF next) with
          | Success next2 -> (parseFX next2)
          | _ -> Failure)
       | _ -> Failure)
