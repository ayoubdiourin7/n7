...
let rec parseE stream =
  ...
  *)
  (print_string "E -> ");
  (match (peekAtFirstToken stream) with
    (* regle #5  - E ->  function ident fleche E { function } *)
    (* Symboles directeurs *)
    |FunctionToken  -> (print_endline "function");(match (accept FunctionToken stream) with
        | Success next -> (match (acceptIdent next) with
           | Success next2 -> (match (accept ArrowToken next2) with
              | Success next3 -> (parseE next3)
              | _ -> Failure)
           | _ -> Failure)
        | _ -> Failure)
    (* regle #2  - E -> if E then E else E { if } *)
    | IfToken -> (print_endline "if");(match (accept IfToken stream) with
...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
      (*  #6  - EX -> = ER EX  | { = } *)
    | EqualToken -> (print_endline "= ER EX");
    (print_endline "= ER EX");
    ...
    (*  - ER -> T TX     | { -, number, ident, true, false, ( } *)

    | IdentToken _ | NumberToken _ | TrueToken | FalseToken |
        MinusToken | LeftParenthesisToken -> (match accept stream with
...
and parseTX stream =
  (print_string "TX -> ");
  (match (peekAtFirstToken stream)h
    (* regle #9  - TX -> + T TX   | { + } *)
    (* regle #10 - TX -> - T TX   | { - } *)
    (* regle #11 - TX ->          | { $, =, ) } *)
