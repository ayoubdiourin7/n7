...
(******************************************************************************)
(*   fonction de suppression d'une liste d'éléments dans un arbre             *)
(*   signature  : retrait_arbre : 'a list -> 'a arbre -> 'a arbre               *)
(*   paramètres : - une liste d'éléments (caractères dans le cas d'un dico)   *)
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire avec le mot supprimé                         *)
(******************************************************************************)
...
let rec retrait_arbre lc (Noeud (b, lb)) =
  ...
  (* sinon on cherche l'arbre arbre_c de la branche correspondant
     ...
     que l'on met à jour en remplacant dans sa liste de branches,
     la branche du premier caractère par la branche dont l'arbre est
     le résultat de la suppression du reste des caractères à l'arbre arbre_c *)
  | c::qlc ->
     let arbre_c =
       let l = recherche c lb in
       match l with
...
(***************************************************************************************)
(*   fonction qui renvoie la liste des mots d'un arbre  l'ordre lexicographique       *)
(*   signature  : liste_mots : 'a arbre -> 'a list list                                   *)
(*   paramètres : - un arbre n-aire                                                     *)
(*   résultat   : la liste des mots de l'arbre dans l'ordre lexicographique             *)
(***************************************************************************************)

let rec liste_mots (Noeud (b, lb)) =
  match lb with
  | [] -> []
  | (c, a)::qlb ->
     let l = liste_mots a in
     let l = List.map (fun x -> c::x) l in
     if b then
       [c]::l @ liste_mots (Noeud (b, qlb))
     else
       l @ liste_mots (Noeud (b, qlb))


let rec deletUselessNoeud(Noeud(b,lb))=
      match b,lb with
      |false,[] -> Noeud(false,[])
      |b,lb -> let list = List.map(fun (e,arbre) -> e,deletUselessNoeud arbre ) lb in 
                  Noeud(b,List.filter (fun (c,b) -> (b <> Noeud(false,[]))) list )
