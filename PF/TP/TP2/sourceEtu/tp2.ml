...
let rec scinde l = match l with
  | [] -> ([],[])
  | [_] -> (l,[])
  | h1::h2::t -> let (l1,l2) = scinde t in (h1::l1,h2::l2)
...
let rec fusionne ordre l1 l2 = 
  ...
  | ([],l) -> l
  | (l,[]) -> l
  | (h1::t1,h2::t2) -> if ordre h1 h2 then h1::(fusionne ordre t1 l2) else h2::(fusionne ordre l1 t2)
...
let test = tri_fusion (fun x y -> x<y) [4;2;4;3;1]
