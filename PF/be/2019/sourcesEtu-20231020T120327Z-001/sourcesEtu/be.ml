...
Fonction qui évalue un polynome en un point
Paramètre x : un réel
Paramètre poly : un polynome codé avec une float list
Résultat : poly(x), un réel
*)

let evalue  x poly = match List.rev poly with
  | a::q -> List.fold_left (fun acc c -> acc *. x +. c) a q
   _ -> assert false

let%test _ = evalue 0. p0 = 2.5
let%test _ = evalue 0. p1 = 2.
let%test _ = evalue 2. p1 = 8.
let%test _ = evalue 3. p1 = 11.
...
let retract  poly =  List.fold_right (fun c acc -> if c=0. && acc=[] then [] else c::acc) poly []  
...
let scal_mult  poly x = List.map (fun c -> c*.x) poly
...
let rec plus   p1 p2= match retract p1,retract p2 with
  | [],[] -> []
  | [],q -> q
  | p,[] -> p
  | a::p,b::q -> (a+.b)::(plus p q)
