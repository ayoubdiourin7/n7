...
struct
  ...
  (*  arbre_vide : ('b,'f) arbre_chiffrement **)
  (*  Arbre de chiffrement vide **)
 ...
 (** let rec get_branche  b arbe  = match arbe with
    | Lettre _ -> None
    | Noeud l -> match  List.assoc_opt b l with
      |None -> List.fold_left (fun acc (x,y) -> match acc with
          |None -> get_branche b y
          |Some _ -> acc
        ) None l
      |Some x -> Some x *)
  ...
  let%test _ = get_branche 'z' arbre4 = None
  ...
  let rec dechiffrer  list arbre = let rec aux list sousArbre = match list with
      |[] -> []
      |t::q -> match get_branche t sousArbre with
        |None -> raise CodeNonValide
        |Some (Lettre x) -> x::(dechiffrer q arbre)
        |Some (Noeud l) -> aux q (Noeud l)
    in aux list arbre
