...
struct
     ...
     *)
    let allocsize (bs, tr) = num_nodes tr 
    ...
    let read (bs, tr) addr = if pow2 bs < addr then raise OutOfBound else (search  tr (bits bs addr))

    (* Implémentation de write. *)
    let write (bs, tr) addr value = if pow2 bs < addr then raise OutOfBound else (bs ,update tr  value (bits bs addr   ))

    (* Implémentation de dump. *)
