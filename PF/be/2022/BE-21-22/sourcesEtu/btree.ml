...
let%test "height-0" = (height (Leaf _0)) = 0
let%test "height-1" = height bt1=   3
...
let rec num_nodes tr = 
    match tr with
    | Leaf _ -> 1
    | Node (l, r) -> 1 + (num_nodes l) + (num_nodes r)
...
let rec num_values tr = match tr with 
    | Leaf _ -> if tr = Leaf _0 then 0 else 1
    | Node (l, r) -> (num_values l) + (num_values r)
...
let rec search tr addrl = match addrl with
    ...
    | x::xs -> (match tr with
        | Leaf x -> _0
        | Node (l, r) -> if x = 0 then search l xs else search r xs)
 ...
 *   devrait pas arriver !
 *)
let rec update tr x addrl =  match addrl with 
    | [] -> Leaf x
    | t::q -> match tr with
        | Node (l, r) -> if t = 0 then Node (update l x q, r) else Node (l, update r x q)
        | Leaf _ -> if t = 0 then Node (sprout x q, Leaf _0) else Node (Leaf _0, sprout x q)
